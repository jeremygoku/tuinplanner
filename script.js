...
async function syncToCloud() {
    ...
    // Haal de achtergrond async uit de nieuwe DB
    ...
    if (f) {
        ...
        r.onload = ev => {
            ...
            img.onload = async () => {
                ...
                try { 
                    await saveToDB('tuinAchtergrondData', compressedData); 
                    sav(); 
                } catch (err) { 
                    alert("⚠️ Opslaan in de database mislukt."); 
                }
            };
...
// ====== INTERACTIES (DRAG & DROP, MUIS, TOUCH) ======
...
// ====== GEÜPDATETE WIS FUNCTIE (LEEGT DATABASE) ======
document.getElementById('btn-clear').onclick = async () => {
    if(confirm("Wissen?")){
        sU();
        plts = [];
        await saveToDB(KEY, plts);
        await saveToDB('tuinAchtergrondData', null);
        D.tA.src = 'tuin.jpeg';
        selIds.clear();
        ren();
    }
...
async function initApp() {
    try {
        // Laad planten uit database
        let savedPlts = await loadFromDB(KEY);
        if (savedPlts) {
            plts = savedPlts;
            plts.forEach(p => {
                if (p.x && p.y && !p.posities) {
                    p.posities = [{ x: p.x, y: p.y }];
        ...
        // Laad achtergrond uit database of fallback naar localStorage
        let savedBg = await loadFromDB('tuinAchtergrondData');
        if (savedBg) {
            D.tA.src = savedBg;
        } else {
            let oldBg = localStorage.getItem('tuinAchtergrondData');
            if (oldBg) {
                D.tA.src = oldBg;
                await saveToDB('tuinAchtergrondData', oldBg);
                localStorage.removeItem('tuinAchtergrondData'); // Opschonen oude methode
            }
        }
    } catch (e) {
        console.error("Fout bij laden van database:", e);
    }
