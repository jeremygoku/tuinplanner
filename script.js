...
async function syncToCloud() {
    ...
    const bD = JSON.stringify({ plants: plts, bg: localStorage.getItem('tuinAchtergrondData') });
    let tk = gapi.client.getToken().access_token;
    
    let query = encodeURIComponent("name='tuinplanner_backup.json' and trashed=false");
    
    if (driveFileId === 'undefined' || driveFileId === 'null') {
        driveFileId = null;
    }
...
async function syncFromCloud() {
    try {
        ...
        if (s.files && s.files.length > 0) {
            ...
            if (d.plants) {
                plts = d.plants;
                if (d.bg) { localStorage.setItem('tuinAchtergrondData', d.bg); D.tA.src = d.bg; }
