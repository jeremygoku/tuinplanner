...
function ren(){let gM=D.fM.value,hM=mnden[new Date().getMonth()];document.getElementById('agenda-titel').innerText=gM==='alle'?"🗓️ Alle Elementen":`📅 Agenda ${gM}`;document.querySelectorAll('.plant-marker').forEach(m=>m.remove());D.aL.innerHTML='';let aL=Array.from(document.querySelectorAll('#laag-filters input:checked')).map(c=>c.value),iW=['December','Januari','Februari'].includes(gM),f=document.createDocumentFragment();
